...
public class RenderUtils2D {
    ...
    public static RenderUtils2D INSTANCE;
    ...
    public static void drawGradientRect(float left, float top, float right, float bottom, int startColor, int endColor, boolean hovered) {
        if (hovered) {
            startColor = ColorUtils.shadeColour(startColor, -20);
            endColor = ColorUtils.shadeColour(endColor, -20);
        }
    ...
    public static void drawBorderedRect(int left, double top, int right, double bottom, int borderWidth, int insideColor, int borderColor, boolean hover) {
        if (hover) {
            insideColor = ColorUtils.shadeColour(insideColor, -20);
            borderColor = ColorUtils.shadeColour(borderColor, -20);
        }
    ...
    public static void drawRoundedRect(double x, double y,
                                       double z, double width, double height, double offset,
                                       int arcSeg, float r, float g, float b) {
        double wsize = width;
        double hsize = height;

        // draw center
        drawQuadFan(x, y, z, wsize - offset, hsize - offset, r, g, b);
        /*
        glBegin(GL_QUADS);
        glVertex3d(x-wsize+offset, y-hsize+offset, z);
        glVertex3d(x+wsize-offset, y-hsize+offset, z);
        glVertex3d(x+wsize-offset, y+hsize-offset, z);
        glVertex3d(x-wsize+offset, y+hsize-offset, z);
        glEnd();
        ...
        glVertex3d(x - wsize + offset, y + hsize - offset, z);
        glVertex3d(x + wsize - offset, y + hsize - offset, z);
        glVertex3d(x + wsize - offset, y + hsize, z);
        glVertex3d(x - wsize + offset, y + hsize, z);
        glEnd();
        ...
        // draw west
        glBegin(GL_QUADS);
        glVertex3d(x - wsize, y - hsize + offset, z);
        glVertex3d(x - wsize + offset, y - hsize + offset, z);
        glVertex3d(x - wsize + offset, y + hsize - offset, z);
        glVertex3d(x - wsize, y + hsize - offset, z);
        glEnd();
        ...
        drawPie(x + wsize - offset, y + hsize - offset, z, offset,
    ...
    public static void drawQuadFan(double x, double y, double z,
                                   double w, double h, float r, float g, float b) {
        ...
        glVertex3d(x, y, z); // center

        if (r != 0f && g != 0 && b != 0) {
            glColor4f(r, g, b, 0);
        }

        glVertex3d(x - w, y - h, z); // lower left
        glVertex3d(x + w, y - h, z); // lower right
        glVertex3d(x + w, y + h, z); // upper right
        glVertex3d(x - w, y + h, z); // upper left

        glVertex3d(x - w, y - h, z); // bring it back
        glEnd();
    }
    public static void drawQuadFan(double x, double y, double z,
                                   double w, double h) {
        drawQuadFan(x, y, z, w, h, 0f, 0f, 0f);
    }
